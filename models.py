...
class Produto(db.Model):
    ...
    tipo = db.Column(db.String(50))  # "combo", "solo", "acompanhamento"
    codigo_de_barra = db.Column(db.String(255))
    disponibilidade = db.Column(db.Boolean)
    preco = db.Column(db.Numeric(10, 2))
    id_und_med = db.Column(db.String(255), nullable=True)  # Defina a relação com Unidade de Medida se necessário
    ...
    etapas = db.relationship('Etapa', backref='produto', lazy='dynamic', cascade='all, delete-orphan')
    acompanhamentos = db.relationship('EtapaAcompanhamento', backref='produto_acompanhamento', lazy='dynamic', cascade='all, delete-orphan')
    ...
    def to_dict(self):
        return {
            ...
            "foto": self.foto,
            "etapas": [etapa.to_dict() for etapa in self.etapas],  # Inclui as etapas
            "acompanhamentos": [acompanhamento.to_dict() for acompanhamento in self.acompanhamentos]  # Inclui os acompanhamentos
        }
...
class EtapaAcompanhamento(db.Model):
    ...
    preco = db.Column(db.Numeric(10, 2))  # Novo campo de preço
    ...
    def to_dict(self):
        return {
            ...
            "preco": str(self.preco)  # Incluindo preço no dicionário
        }
